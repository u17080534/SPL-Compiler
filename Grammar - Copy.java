...
public class Grammar
{
	...
	public AbstractSyntaxTree build(List<Token> stream) throws SyntaxException
	{
		...
		try
        {
           	...
           	if(this.index + 1 < this.tokenstream.size())
           		throw new SyntaxException(this.current, "There are unused tokens...\n\tPlease check your program structure before retrying");
	...
	private Expression PROC_DEFS_() throws SyntaxException
	{
		try
		{
			...
			else if(this.lookahead == Token.Tok.NULL || this.lookahead == Token.Tok.TOK_CB)
		...
		throw new SyntaxException(this.current, "Procedural Error: proc definition is followed by an unrecognized instruction");
	}
		...
		try
		{
			if(this.lookahead == Token.Tok.TOK_SEMI)
			{
				...
				if(this.lookahead == Token.Tok.TOK_PROC)
				{
					...
					System.out.println("Lexical Warning! Final instruction in code block has a trailing semicolon (;) " + this.current);
				else
				{
					Expression e = CODE();
					return new code_(e);
